...
(function($) {
    $.fn.couchLogin = function(opts) {
        ...
        elem.delegate('input[value="Cancel"]', "click", function() {
            elem.html(templates.loggedOut);
        });
    ...
    var templates = {
        ...
        loginForm : '<form class="login" method=”GET” enctype=”text/plain”><label for="name">Name</label> <input type="text" name="name" value="" autocapitalize="off" autocorrect="off"><br><label for="password">Password</label> <input type="password" name="password" value=""><br><input type="submit" value="Login"><input type="submit" value="Cancel"></form>',
        signupForm : '<form class="signup"><label for="name">Name</label> <input type="text" name="name" value="" autocapitalize="off" autocorrect="off"><br><label for="password">Password</label> <input type="password" name="password" value=""><input type="submit" value="Signup"><a href="#login">or Login</a></form>'
    };
    function loggedIn(r) {
        var auth_db = encodeURIComponent(r.info.authentication_db)
        , uri_name = encodeURIComponent(r.userCtx.name)
        , span = $('<span>Welcome <span   class="name"></span>! <a href="#logout" id="logout">Logout?</a></span>');
        $('span.name', span).text(r.userCtx.name); // you can get the user name here
        return span;
    }
})(jQuery);
