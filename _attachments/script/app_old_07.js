$(function(){
			...
			this.JSONstatResults={};
				...
				$.ajax({ 
                    ...
                    success:function(response){
						...
						console.log(JSON.stringify(riResponseList));
	...
	window.GraphView=  Backbone.View.extend({
		...
		createSerie: function (groupedResults,yValue){ //returns an xAxis object and an serie object (see highcharts.com)
			...
			//~ console.log(groupedResults,yValue);
							...
							//TEXT TYPES: put every value as key of an object with value 1, like {ingv:1, geofon:1}, then sums the values
							 if (sumType=='Text' || sumType=='Select'){
								 if (!tempObj.hasOwnProperty(tempKey)){
									 tempObj[tempKey]=1;
									 serieJSON.data[counter]=serieJSON.data[counter]+1;
									 }
								 }
							
							//LIST TYPES: count the number of elements for each list
							if (sumType=='List'){
								serieJSON.data[counter]=serieJSON.data[counter]+tempKey.length;
								}
							
							//NUMBER TYPES: simply sums the numbers
							if (sumType=='Number'){
								var addValue= (_.isNumber(tempKey))? (tempKey): 0;
								 serieJSON.data[counter]=serieJSON.data[counter]+addValue;
								}
							}
						});
					
					//rounds the number in case there are decimal places
					serieJSON.data[counter]=Math.floor(serieJSON.data[counter]);
    ...
    window.mapBox = Backbone.View.extend({
        ...
        showRIsOnMap:function(){
            ...
            var couchDBqueries=[labCoordsView,netStationsCoordsView,GPSCoordsView];
