   ...
   financial_contact_person_institution: {}
	...
	window.RUView = Backbone.View.extend({
		...
		updateRI:function(){
			...
			//takes RI data and put it in a form
			var jsondata=(json_infos.RI).pop();
			this.form=new Backbone.Form({
				data: jsondata, //Data to populate the form with
				schema: self.getSchema(jsondata),
				}).render();
			$(this.form.el).prepend("<center><b>"+self.renderIMG()+self.model.get('ri_name')+"</b></center>").append('<div id="updatebtn">OK, Save all Changes!</div> <div id="discard"> Close Popup to Discard Changes</div>');
		...
		saveRI:function(){
			...
			success:function(){
			    $('#updatebtn').append('<div id="savemsg" style="color:green;top:-100px; position:relative;"/>');
			    $('#savemsg').append('   <span style="top:-40px; position:relative;"> <b id="iwillfade" style="color:red"> Doc succesfully saved!</b></span>');
			...
			for (row in otherDocs.rows){
				...
				$.couch.db(dbName).saveDoc(otherDocs.rows[row].value,{
				    success:function(){
					$('#savemsg').prepend('   <div id="iwillfade'+saved+'" style="color:green; position:relative;top:-200px;"> <b > Saved '+saved+'/'+otherDocs.rows.length+' </b></div>');
					$('#iwillfade'+saved+'').delay(1000).fadeOut(500);
					saved=saved+1;
					},
				    error:function(){
					$('#savemsg').prepend('   <div id="iwillfade'+saved+'" style="color:green; position:relative;top:-200px;"> <b > Error on doc '+saved+'/'+otherDocs.rows.length+' with id '+otherDocs.rows[row].value.ri_name+'</b></div>');
					saved=saved+1;
					}
				    });
				}
		...
		getSchema:function(JSONobj){
		    ...
		    //the general schema is in the backboneJSONform  global var
