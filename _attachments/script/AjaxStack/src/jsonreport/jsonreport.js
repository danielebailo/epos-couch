...
//~ var cssText =
  ...
  //~ ".jsonreport TD { padding: 4px 4px 0 4px; vertical-align: top; }\r\n" +
  //~ ".jsonreport DL { clear: left; }\r\n" +
  //~ ".jsonreport DT { margin: 5px 0 2px 0; font: bold 18px Helvetica, Verdana, Arial; width: 200px; clear: left; float: left; display:block;  word-wrap:break-word;}\r\n" +
  ...
  //~ ".jsonreport TBODY>TR:nth-child(even) {background: #CCC}"+".jsonreport TBODY>TR:nth-child(odd) {background: #FFF}"
...
_.jsonreport = (function(){
    ...
    function obj(m) {
    var sb = '<TABLE class="without-border">';   
    for (var k in m) sb += '<TR><TD class="rightalign">' + splitCase(k) + ': </TD><TD>' + val(m[k]) + '</TD></tr>';
    sb += '</TABLE>';
    return sb; 
        
        
        
      //~ var sb = '<dl>';   ***ORIGINAL VERSION**
    ...
    function arr(m) {
      ...
      var sb = '<table class="with-border" id="tbl-' + id + '"><caption></caption><thead><tr>';
      tbls.push(m);
      var i=0;
